...
Если у вас установлен **Docker** и **Docker Compose**, проект можно запустить одной командой.

```bash
cd skill-swap-team-project
docker compose up --build
```

После запуска:

- Бэкенд (Laravel API): `http://localhost:8000`
...
Laravel использует эти параметры подключения через переменные окружения в `docker-compose.yml` (`DB_HOST=db` и т.д.).
